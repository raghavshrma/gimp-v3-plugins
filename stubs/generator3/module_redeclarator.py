        ...
        if self.split_modules:
            ...
            with fopen(os.path.join(last_pkg_dir, "__init__.py"), "w") as init:
                ...
                for buf in self.classes_buffs:
                    with fopen(
                        os.path.join(last_pkg_dir, buf.name) + ".py", "w"
                    ) as dummy:
            ...
            last_pkg_dir = build_pkg_structure(
                self.cache_dir, ".".join(qname_parts[:-1])
            )
            ...
            existing_pkg_init = os.path.join(
                last_pkg_dir, qname_parts[-1], "__init__.py"
            )
    ...
    def redo_imports(self):
        ...
        for item_name in self.module.__dict__.keys():
            ...
            if (
                ...
                ref_notice = (
                    getattr(item, "__file__", str(item))
                    if not self.test_mode
                    else "<ref>"
                )
    ...
    def redo(self, p_name, inspect_dir):
        ...
        for item_name in module_dict:
            ...
            maybe_import_mod_name = (
                mod_name if isinstance(mod_name, type(p_name)) else ""
            )

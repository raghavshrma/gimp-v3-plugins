...
def required_gen_version_file_path():
    return os.environ.get(
        ENV_REQUIRED_GEN_VERSION_FILE, os.path.join(_parent_dir, "required_gen_version")
    )
...
def is_skipped_module(path, f, qname):
    return (
        ...
        or is_tensorflow_contrib_ops_module(qname)
    )
...
def is_module(d, root):
    return (
        ...
        or os.path.exists(os.path.join(root, d, "__init__.pyi"))
        or os.path.exists(os.path.join(root, d, "__init__.pyo"))
        or is_valid_implicit_namespace_package_name(d)
    )
...
def skeleton_status(base_dir, mod_qname, mod_path, sdk_skeleton_state=None):
    ...
    if "gen_version" not in skeleton_meta:
        ...
        blacklist_record = read_failed_version_and_mtime_from_legacy_blacklist(
            base_dir, mod_path
        )
        if blacklist_record:
        ...
        existing_skeleton_version = read_used_generator_version_from_skeleton_header(
            base_dir, mod_qname
        )
    ...
    if (
        mod_path
        and used_bin_mtime is not None
        and used_bin_mtime < file_modification_timestamp(mod_path)
    ):
        return SkeletonStatus.OUTDATED

    if skeleton_meta.get("status") == GenerationStatus.FAILED:
        return (
            SkeletonStatus.OUTDATED
            if used_version < gen_version
            else SkeletonStatus.FAILING
        )
    ...
    if required_version and used_version:
        return (
            ...
            else SkeletonStatus.UP_TO_DATE
        )
...
def read_failed_version_from_stamp(base_dir, mod_qname):
    ...
    with ignored_os_errors(errno.ENOENT):
        with fopen(
            os.path.join(base_dir, FAILED_VERSION_STAMP_PREFIX + mod_qname), "r"
        ) as f:
        ...
        with fopen(os.path.join(sdk_skeletons_dir, ".blacklist"), "r") as f:
            for line in f:
                ...
                if m:
                    ...
                    results[bin_path] = (
                        version_to_tuple(m.group("version")),
                        bin_mtime,
                    )
    return results
...
def create_failed_version_stamp(base_dir, mod_qname):
    failed_version_stamp = os.path.join(
        base_dir, FAILED_VERSION_STAMP_PREFIX + mod_qname
    )
...
class SkeletonGenerator(object):
    ...
    def discover_and_process_all_modules(self, name_pattern=None, builtins_only=False):
        ...
        if not builtins_only:
            ...
            all_modules.extend(
                sorted(self.discover_binary_modules(), key=(lambda b: b.qname))
            )

        matching_modules = [
            m for m in all_modules if fnmatch.fnmatchcase(m.qname, name_pattern)
        ]

        progress("Updating skeletons...")
        for i, mod in enumerate(matching_modules):
            progress(
                text=mod.qname, fraction=float(i) / len(matching_modules), minor=True
            )
    ...
    def process_module(self, mod_name, mod_path=None):
        ...
        if self.in_state_json:
            existing_skeleton_meta = self.in_state_json["sdk_skeletons"].get(
                mod_name, {}
            )
            sdk_skeleton_state = self.out_state_json["sdk_skeletons"][mod_name] = (
                deepcopy(existing_skeleton_meta)
            )
        else:
            sdk_skeleton_state = self.out_state_json["sdk_skeletons"][mod_name] = {}
    ...
    def reuse_or_generate_skeleton(self, mod_name, mod_path, mod_state_json):
        ...
        try:
            sdk_skeleton_status = skeleton_status(
                self.output_dir, mod_name, mod_path, mod_state_json
            )
            ...
            cached_skeleton_status = skeleton_status(
                mod_cache_dir, mod_name, mod_path, mod_state_json
            )
            if cached_skeleton_status == SkeletonStatus.OUTDATED:
                return execute_in_subprocess_synchronously(
                    name="Skeleton Generator Worker",
                    func=generate_skeleton,
                    args=(mod_name, mod_path, mod_cache_dir, self.output_dir),
                    kwargs={},
                    failure_result=GenerationStatus.FAILED,
                )
            elif cached_skeleton_status == SkeletonStatus.FAILING:
                logging.info(
                    "Cache entry for %s at %r indicates failed generation",
                    mod_name,
                    mod_cache_dir,
                )
                return GenerationStatus.FAILED
                ...
                logging.info(
                    "Copying cached stubs for %s from %r to %r",
                    mod_name,
                    mod_cache_dir,
                    self.output_dir,
                )
                copy_skeletons(
                    mod_cache_dir,
                    self.output_dir,
                    get_module_origin(mod_path, mod_name),
                )
...
def generate_skeleton(name, mod_file_name, mod_cache_dir, output_dir):
    ...
    if redo_imports:
        ...
        for m in list(sys.modules):
            ...
            if m not in imported_module_names and not hasattr(
                ...
                try:
                    redo_module(
                        m, mod_file_name, cache_dir=mod_cache_dir, output_dir=output_dir
                    )
                    extra_modules = set(sys.modules) - initial_module_set
                    if extra_modules:
                        report(
                            "Introspecting submodule %r of %r led to extra content of sys.modules: %s",
                            m,
                            name,
                            ", ".join(extra_modules),
                        )
...
def redo_module(module_name, module_file_name, cache_dir, output_dir):
    ...
    if mod:
        ...
        copy_skeletons(
            cache_dir, output_dir, get_module_origin(module_file_name, module_name)
        )
    else:

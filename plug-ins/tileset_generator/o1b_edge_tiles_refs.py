...
def _setup_sources(src: QuickBuilder | Builder):
    src.setup_sample()
...
def _h_first(t: TilesetTargetGroup, src: QuickBuilder | Builder):
    key = "l1-edge-h-seamless"
    t.add(1, 1, src.copy_block_custom(key, 1, 1, 3, 3))

def _h_nth(t: TilesetTargetGroup, src: QuickBuilder | Builder, index: int):
    t.add(2, 1, src.sample_edge_top_full(index))
    t.add(2, 3, src.sample_edge_bottom(index))

def _v_first(t: TilesetTargetGroup, src: QuickBuilder | Builder):
    key = "l1-edge-v-seamless"
    t.add(1, 1, src.copy_block_custom(key, 1, 1, 3, 3))

def _v_nth(t: TilesetTargetGroup, src: QuickBuilder | Builder, index: int):
    t.add(1, 2, src.sample_edge_left(index))
    t.add(2, 2, src.deep_dark())
    t.add(3, 2, src.sample_edge_right(index))

def _setup_ref_sources(src: QuickBuilder | Builder):
    src.custom_source("l1-edge-h-1")
    src.custom_source("l1-edge-v-1")

def _h_ref(t: TilesetTargetGroup, src: QuickBuilder | Builder):
    key = "l1-edge-h-1"
    t.add(1, 1, src.copy_block_custom(key, 2, 1, 1, 3))
    t.add(3, 1, src.copy_block_custom(key, 2, 1, 1, 3))

def _v_ref(t: TilesetTargetGroup, src: QuickBuilder | Builder):
    key = "l1-edge-v-1"

...
def _build_slopes_6(bld: V1SourceSet, idx: int, x: int, y: int):
    t_raw = _get_raw_group(bld, idx, x, y)
    _add_raw_source(bld, t_raw, 2)
    t_raw.add(2, 2, bld.in_corner_tl_ext())
    t_raw.add(4, 2, bld.in_corner_tr_ext())
    t_raw.finalize()
...
def _build_slopes_7(bld: V1SourceSet, idx: int, x: int, y: int):
    t_raw = _get_raw_group(bld, idx, x, y)
...
def _build_slopes_8(bld: V1SourceSet, idx: int, x: int, y: int):
    t_raw = _get_raw_group(bld, idx, x, y)
...
def _build_slopes_9(bld: V1SourceSet, idx: int, x: int, y: int):
    t_raw = _get_raw_group(bld, idx, x, y)
    t_raw.add(2, 2, [bld.in_corner_tl()])
    t_raw.add(4, 2, [bld.in_corner_tr()])
    t_raw.add(2, 4, [bld.in_corner_bl()])
    t_raw.add(4, 4, [bld.in_corner_br()])
    t_raw.finalize()
    ...
    t_ref.add(3, 2, bld.deep_light())
    t_ref.add(5, 1, source.copy_block2(4, 1, 1, 2))

    t_ref.add(1, 3, bld.edge_left())
    t_ref.add(2, 3, bld.deep_edge_left())
    t_ref.add(3, 3, bld.deep_dark())
    t_ref.add(4, 3, bld.deep_edge_right())
    t_ref.add(5, 3, bld.edge_right())

    t_ref.add(1, 4, source.copy(2, 4))
    t_ref.add(3, 4, bld.deep_light())

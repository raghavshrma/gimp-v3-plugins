...
def _build_slopes_1(bld: Builder, idx: int, x: int, y: int):
    ...
    t_raw.add(2, 1, bld.outer_corner_tl_extra())
    t_raw.add(2, 2, bld.single_h_left())
    t_raw.add(4, 1, [bld.outer_corner_tr_extra()])
    t_raw.add(4, 2, [bld.single_h_right()])
    t_raw.add(2, 3, [bld.outer_corner_tl_extra()])
    t_raw.add(2, 4, [bld.single_h_left()])
    t_raw.add(4, 3, [bld.outer_corner_tr_extra()])
...
def _build_slopes_3(bld: Builder, idx: int, x: int, y: int):
    t_raw = _get_raw_group(bld, idx, x, y)
    t_raw.add(2, 1, bld.outer_corner_tl_extra())
